...
import com.desafio.alura.foro.domain.respuesta.validations.create.ValidarRespuestaCreada;
import com.desafio.alura.foro.domain.respuesta.validations.update.ValidarRespuestaActualizada;
...
@Tag(name = "Respuesta", description = "Solo una puede ser la solucion al tema.")
public class RespuestaController {
    ...
    leerRespuestaDeTopico(@PageableDefault(size = 5, sort = {"ultimaActualizacion" },
            ...
            direction = Sort.Direction.ASC) Pageable pageable, @PathVariable Long topicold) {
        var pagina = respuestaRepository.findAllByTopicoId(topicold, pageable).map(DetalleRespuestaDTO::new);
    ...
    leerRespuestasDeUsuarios(@PageableDefault(size = 5, sort = {"ultimaActualizacion" },
            direction = Sort.Direction.ASC) Pageable pageable, @PathVariable Long usuariold) {
        var pagina = respuestaRepository.findAllByUsuarioId(usuariold, pageable).map(DetalleRespuestaDTO::new);
        return ResponseEntity.ok(pagina);
    }
